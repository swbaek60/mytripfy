      ...
      <div className="bg-surface rounded-2xl shadow-sm p-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
          {visits.map((v) => {
            ...
            return (
              <div key={v.id} className="group relative rounded-xl border border-edge overflow-hidden hover:border-success-border hover:shadow-md transition-all">
                <Link href={`/${locale}/sponsors/${v.sponsor_id}`} className="block">
                  <div className="aspect-square bg-surface-sunken relative group/img">
                    ...
                    />
                    <div className="absolute top-1.5 left-1.5 bg-warning-strong text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full">
                      +{v.points_granted}pt
                    </div>
                    {!isOwnProfile && canDispute && !alreadyDisputed && (
                      <button
                        ...
                        className="absolute top-1 right-1 w-6 h-6 bg-danger/90 hover:bg-danger text-white rounded-full flex items-center justify-center opacity-0 group-hover/img:opacity-100 transition-all shadow-lg z-10"
                        title={t('report')}
                      >
                    ...
                    {alreadyDisputed && (
                      <div className="absolute top-1 right-1 bg-subtle/80 text-white text-[9px] font-bold px-1.5 py-0.5 rounded-full">
                        {t('reported')}
                      </div>
                    )}
                  </div>
        ...
        <Link
          ...
          className="inline-block mt-4 text-sm font-semibold text-success hover:text-success-strong"
        >
          {t('viewAllSponsors')} →
        </Link>
      </div>
      ...
      <div className="relative bg-surface rounded-3xl overflow-hidden w-full max-w-md shadow-2xl">
        <div className="bg-gradient-to-r from-danger to-sunset-strong px-6 py-5 text-white">
          <div className="flex items-center justify-between">
            ...
            <button type="button" onClick={() => !submitting && onClose()} aria-label={tc('close')} className="w-8 h-8 bg-surface/20 rounded-full flex items-center justify-center hover:bg-surface/30">✕</button>
          </div>
        </div>
        <div className="p-6">
          {success ? (
            ...
            <>
              ...
              {error && <div className="mt-3 text-danger-strong text-sm bg-danger-light p-3 rounded-xl">{error}</div>}
              <div className="flex gap-2 mt-4">
                ...
                <button
                  ...
                  className="flex-1 bg-danger text-white font-bold py-3 rounded-xl hover:bg-danger text-sm disabled:opacity-40 flex items-center justify-center gap-1.5"
                >
