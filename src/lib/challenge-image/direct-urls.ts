/**
 * 챌린지 카테고리별 "직접 이미지 URL" 표 (서버 전용).
 *
 * 예전에는 이 표들이 ChallengeImage.tsx('use client') 안에 있어서 약 390KB 짜리
 * 청크가 브라우저로 내려갔다 — 앱 전체에서 가장 큰 클라이언트 청크였다.
 * 제목·카테고리는 서버가 이미 알고 있으므로 여기서 URL 을 확정해 prop 으로 내려보낸다.
 *
 * `getDirectImageUrl` 은 순수 함수이므로 같은 입력이면 항상 같은 결과다.
 * 따라서 컴포넌트가 렌더 중 여러 번 호출하던 것을 prop 하나로 대체해도 동작이 같다.
 */
import { cleanTitle, simpleHash, toSlug } from './title'
...
export function getDirectImageUrl(category: string, titleEn: string): string | undefined {
  const normalized = titleEn.replace(/\u2018|\u2019|\u201A/g, "'").trim()
