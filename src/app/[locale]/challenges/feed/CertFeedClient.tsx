...
export default function CertFeedClient({
  ...
  const handleDispute = async () => {
    ...
    setError('')
    try {
      await api.post('/api/challenges/dispute', {
      ...
      setError(errorMessage(e, tc('reportFailed')))
    } finally {
      setSubmitting(false)
    }
  }
    ...
    <div>
      ...
      <div className="flex flex-wrap gap-2 mb-5">
        <input
          ...
          className="flex-1 min-w-[160px] border border-edge rounded-xl px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-challenge/40 bg-surface"
        />
        {[
          ...
          <button
            ...
            className={`px-3 py-2 rounded-xl text-xs font-semibold border transition-colors ${
              filterStatus === f.key
                ...
                : 'bg-surface text-body border-edge hover:border-challenge-border'
            }`}
          >
            {f.label}
          </button>
              ...
              >
                ...
                <div className="p-4">
                  ...
                  <div className="flex items-center gap-2 mb-3">
                    {cert.avatar_url ? (
                      <Avatar src={cert.avatar_url} name={cert.full_name} size={32} />
                    ) : (
                      <div className="w-8 h-8 rounded-full bg-gradient-to-br from-challenge to-challenge flex items-center justify-center text-white text-xs font-bold">
                  ...
                  <div className="mt-3 flex gap-2">
                    {cert.dispute_status === 'reviewing' && (
                      <Link
                        ...
                        className="flex-1 text-center bg-challenge-light border border-challenge-border text-challenge-strong text-xs font-bold py-2 rounded-xl hover:bg-challenge-muted transition-colors"
                      >
                        {tc('juryVoteShort')}
                      </Link>
                    )}
                    {isOwn && cert.dispute_status !== 'clean' && (
                      <div className={`flex-1 text-center text-xs font-semibold py-2 rounded-xl ${status.bg} ${status.color}`}>
                        {tc('myCert')} · {status.label}
                      </div>
                    )}
                    {isOwn && cert.dispute_status === 'clean' && (
                      <div className="flex-1 text-center bg-success-light text-success-strong text-xs font-semibold py-2 rounded-xl">
                        {L.status.clean} {tc('myCert')}
                      </div>
                    )}
                    ...
                    {canFlag && (canDispute || !currentUserId) && (
                      <button
                        onClick={() => {
                          if (!currentUserId) { router.push(`/${locale}/login?returnTo=${encodeURIComponent(window.location.pathname)}`); return }
                        ...
                        className="flex-1 flex items-center justify-center gap-1 bg-danger-light border border-danger-border text-danger-strong text-xs font-bold py-2 rounded-xl hover:bg-danger-light transition-colors"
                      >
                        <Siren className="w-3.5 h-3.5" />{L.buttonText}
                      </button>
                    )}
                    {canFlag && currentUserId && !canDispute && (
              ...
              {successMsg ? (
                ...
                <>
                  ...
                  <div className="relative h-36 rounded-2xl overflow-hidden mb-4">
                    <SmartImage src={disputeTarget.image_url} alt="" width={448} height={144} sizes="(max-width: 480px) 100vw, 448px" className="w-full h-full object-cover" />
                  ...
                  <label className="block text-sm font-bold text-heading mb-2">
                    {tc('reason')} <span className="text-danger">*</span>
                    <span className="font-normal text-hint ml-1">{tc('minChars', { count: 10 })}</span>
                  </label>
                  <textarea
                    value={reason}
