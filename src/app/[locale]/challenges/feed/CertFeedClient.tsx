...
export default function CertFeedClient({
  ...
  const STATUS_UI = {
    clean:       { label: L.status.clean,       color: 'text-green-700', bg: 'bg-green-50' },
    flagged:     { label: L.status.flagged,     color: 'text-amber-700', bg: 'bg-amber-50' },
    reviewing:   { label: L.status.reviewing,   color: 'text-blue-700',  bg: 'bg-blue-50' },
    invalidated: { label: L.status.invalidated, color: 'text-red-700',   bg: 'bg-red-50' },
    ...
    <div>
      ...
      <div className="flex flex-wrap gap-2 mb-5">
        <input
          ...
          className="flex-1 min-w-[160px] border border-gray-200 rounded-xl px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-purple-300 bg-white"
        />
        {[
          { key: 'all',       label: 'All' },
              ...
              >
                ...
                <div className="p-4">
                  ...
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <div>
                      <p className="text-sm font-semibold text-gray-900 leading-none">{cert.full_name}</p>
                      <p className="text-[10px] text-gray-400 mt-0.5 capitalize">{cert.category}</p>
                    </div>
                  </div>

                  {/* 챌린지명 (locale별 번역) */}
                  <p className="text-sm font-bold text-gray-800 line-clamp-1">{cert.title ?? cert.title_en}</p>
                  ...
                  <div className="mt-3 flex gap-2">
                    {cert.dispute_status === 'reviewing' && (
                      <Link
                        ...
                        className="flex-1 text-center bg-blue-50 border border-blue-200 text-blue-700 text-xs font-bold py-2 rounded-xl hover:bg-blue-100 transition-colors"
                      >
                    ...
                    {isOwn && cert.dispute_status === 'clean' && (
                      <div className="flex-1 text-center bg-green-50 text-green-600 text-xs font-semibold py-2 rounded-xl">
                        {L.status.clean} My cert
                      </div>
                    )}
                    {!isOwn && cert.already_disputed && (
                      <div className="flex-1 text-center bg-gray-50 text-gray-400 text-xs font-semibold py-2 rounded-xl">
                        {L.alreadyFlagged}
                      </div>
                    )}
                    {canFlag && (
                      <button
                        ...
                        className="flex-1 flex items-center justify-center gap-1 bg-red-50 border border-red-200 text-red-600 text-xs font-bold py-2 rounded-xl hover:bg-red-100 transition-colors"
                      >
                        <Siren className="w-3.5 h-3.5" />{L.buttonText}
                      </button>
                    )}
                    {!isOwn && currentUserId && myCertCount < 3 && !cert.already_disputed && (
                      <div className="flex-1 text-center bg-gray-50 text-gray-300 text-xs py-2 rounded-xl">
                        Need 3 certs
                      </div>
                    )}
                  </div>
            ...
            <div className="p-6">
              {successMsg ? (
                <div className="text-center py-6">
                  <div className="flex justify-center mb-3"><Siren className="w-14 h-14 text-red-500" /></div>
                ...
                <>
                  ...
                  <label className="block text-sm font-bold text-gray-900 mb-2">
                    ...
                    <span className="font-normal text-gray-400 ml-1">(min 10 chars)</span>
                  </label>
                  <textarea
                    value={reason}
                  ...
                  <div className="flex gap-2 mt-4">
                    ...
                    <button
                      ...
                      className="flex-1 bg-red-500 text-white font-bold py-3 rounded-xl hover:bg-red-600 transition-colors text-sm disabled:opacity-40 disabled:cursor-not-allowed"
                    >
                      {submitting ? (
