...
const TYPE_KEYS: Record<string, string> = {
  ...
  answer: 'type_answer',
  guide_match: 'type_guide_match',
  guide_request_match: 'type_guide_request_match',
  guide_application: 'type_guide_application',
}

function getTypeLabel(
  n: Pick<Notification, 'type' | 'reference_type'>,
  t: (key: string) => string
) {
  if (n.type === 'message' && n.reference_type === 'group_chat') return t('type_group')
  const key = TYPE_KEYS[n.type]
  return key ? t(key) : t('type_notice')
}
        ...
        <div className="flex-1 overflow-y-auto">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center h-48 gap-3">
              ...
              <p className="text-sm text-hint">{t('empty')}</p>
            </div>
          ) : (
