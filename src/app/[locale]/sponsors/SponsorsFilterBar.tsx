      ...
      <div ref={countryRef} className="relative">
        ...
        >
          {selectedCountry ? (
            ...
            <span className="text-subtle">{t('filterCountry')} · {t('allTypes')}</span>
          )}
          <ChevronDown className={`w-4 h-4 shrink-0 ml-auto transition-transform ${countryOpen ? 'rotate-180' : ''}`} />
        </button>
        {countryOpen && (
          <div className="absolute left-0 top-full mt-1 bg-surface rounded-xl shadow-lg border border-edge z-50 overflow-hidden min-w-[220px]">
