...
/**
 * 소셜 로그인: <a target="_self" rel="noopener noreferrer">로 같은 탭 이동만 허용.
 * form 대신 링크 사용 시 모바일에서 같은 탭 동작이 더 안정적일 수 있음.
 */
export default function SocialLoginButtons({ locale }: Props) {
  const q = `provider=google&locale=${encodeURIComponent(locale)}`
  const g = `${BASE}?${q}`
  const a = `${BASE}?${q.replace('provider=google', 'provider=apple')}`
  const f = `${BASE}?${q.replace('provider=google', 'provider=facebook')}`
  ...
  return (
    <div className="space-y-3">
      <a
        ...
        className={`${linkBase} border-gray-200 bg-white hover:bg-gray-50 hover:border-gray-300 text-gray-700`}
      >
      ...
      <a
        ...
        target="_self"
        rel="noopener noreferrer"
        className={`${linkBase} border-gray-900 bg-gray-900 hover:bg-black text-white`}
      >
      ...
      <a
        ...
        target="_self"
        rel="noopener noreferrer"
        className={`${linkBase} border-[#1877F2] bg-[#1877F2] hover:bg-[#166FE5] text-white`}
      >
        <svg width="20" height="20" viewBox="0 0 24 24" fill="white" className="shrink-0">
